...
class Product implements AST
{
    ...
    public Quantity eval(Map<String,Quantity> env)
    {
        Quantity lterm = left.eval(env);
        Quantity rterm = right.eval(env);
        return lterm.mul(rterm);
    }
...
class Normalize implements AST
{
    ...
    {
        Quantity term = child.eval(env);
        System.out.println( "Evaluating " + child + " and got " + term );
        return term.normalize(env);
    }
...
class Define implements AST
{
    ...
    public Quantity eval(Map<String,Quantity> env)
    {
        Quantity term = defn.eval(env);
        env.put(unitName,term);
